...
export class PhpcsConfiguration extends Disposable {
	...
	public async compute(params: Proposed.ConfigurationParams, _token: CancellationToken, _next: Function): Promise<any[]> {
		...
		for (let item of params.items) {
			...
			settings = await this.resolveExecutablePath( folder? folder.uri.fsPath : null, settings);
	...
	protected async resolveExecutablePath(workspacePath: string, settings: PhpcsSettings): Promise<PhpcsSettings> {
		if (settings.executablePath === null) {
			let executablePathResolver = new PhpcsPathResolver(workspacePath, settings);
...
class ComposerPathResolver extends BasePathResolver {
	...
	protected readonly _workspacePath: string;
	 ...
	 */
	constructor(workspacePath: string, workingPath?: string) {
		super();

		this._workspacePath = workspacePath;

		if (!path.isAbsolute(workingPath)) {
			workingPath = path.join(workspacePath, workingPath);
		}
...
class PhpcsPathResolver extends BasePathResolver {
	...
	constructor(workspacePath: string, settings: PhpcsSettings) {
		...
		if (workspacePath !== null) {
			this.resolvers.push(new ComposerPathResolver(workspacePath, settings.composerJsonPath));
		}
